...
namespace DoctorPremium.Web.Helpers
{
	...
	{
		private static readonly List<string> validLanguages = new List<string> { "en", "ru" };
		private static string currentLanguage { get; set; }

		public static string GetCurrentLanguage()
		{
			return (String)HttpContext.Current.Session["lang"] ?? GetDefaultLanguage();
		}

		public static int GetCurrentLanguageId()
		{
			return (int)Enum.Parse(typeof(LanguageEnum), GetCurrentLanguage(), true);
		}
		...
		public static bool IsRelevantLanguage(string lang)
		{
		...
		{
			return ((LanguageEnum)1).ToString();
		}
		...
		public static bool IsNullCurrentLanguage()
		{
			return HttpContext.Current.Session == null || HttpContext.Current.Session["lang"] == null;
		}

        public static List<SelectListItem> GetLanguageDropList(ILanguageService _languageService)
        {	
            List<SelectListItem> selectList = _languageService.GetAllLanguages().Select(x => new SelectListItem() { Text = x.LanguageName, Value = x.LanguageId.ToString() }).ToList();
            return selectList;
        }
